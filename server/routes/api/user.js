...
.post(async (req,res)=>{
    ...
    const token = user.generateTokenForgotPassword()

    const link = `http://localhost:3001/api/users/forgot-password/${user._id}/${token}`
...
router.route('/forgot-password/:userid/:token')
.get(async (req,res)=>{
    try{
        const {userid, token} = req.params

        const jwt = require('jsonwebtoken')
        const { _id,email,exp } = await jwt.verify(token, process.env.DB_SECRET)
        ...
        const user_token = user.generateToken()

        res.cookie('x-access-token', user_token)
        .status(200).send(getUserProps(user))
    } catch(error) {
        res.status(400).json({message: 'Problem occured while updating profile', error: error})
    }
